...
/**
 * Bind functionality for the profile selection buttons. If another profile
 * is selected, the UI of the previously selected profile will be updated.
 */
function bindProfileSelect() {
    Array.from(document.getElementsByClassName('settingsProfileSelect')).map((val) => {
        val.onclick = (e) => {
            if (val.hasAttribute('selected')) {
                return
            }
            const selectBtns = document.getElementsByClassName('settingsProfileSelect')
            for (let i = 0; i < selectBtns.length; i++) {
                if (selectBtns[i].hasAttribute('selected')) {
                    selectBtns[i].removeAttribute('selected')
                    selectBtns[i].innerHTML = 'Select Profile'
                }
            }
            val.setAttribute('selected', '')
            val.innerHTML = 'Selected Profile &#10004;'
            ConfigManager.setSelectedServer(val.closest('.settingsProfile').getAttribute('uuid'))
        }
    })
}
...
function bindVersionSelect() {
    ...
    val.onchange = (e) => {
        ...
        VersionManager.setType(0, type)
        VersionManager.save()
    }
}

function bindFabricSelect() {
    let val = document.getElementById('fabricBool')
    val.onclick = (e) => {
        ConfigManager.setUsingFabric(val.checked)
    }
}
 ...
 */
function bindProfileDelete() {
    Array.from(document.getElementsByClassName('settingsProfileDelete')).map((val) => {
        val.onclick = (e) => {
            let isLastProfile = false
            if (VersionManager.amountOfProfiles() === 1) {
                isLastAccount = true
                setOverlayContent(
                    'Warning<br>This is Your Last Profile',
                    'In order to launch the game you must have at least one profile. You will need to create a new profile.<br><br>Are you sure you want to delete the profile?',
                    'I\'m Sure',
                    'Cancel'
                )
                setOverlayHandler(() => {
                    processDelete(val, isLastProfile)
                    toggleOverlay(false)
...
/**
 * Process a profile delete.
 * 
 * @param {Element} val The delete button element.
 * @param {boolean} isLastAccount If this logout is on the last added account.
 */
function processDelete(val, isLastAccount) {
    const parent = val.closest('.settingsProfile')
    const uuid = parent.getAttribute('uuid')
    const prevSelAcc = VersionManager.getProfile(uuid)

    if (isLastAccount) {
        VersionManager.removeProfile(prevSelAcc.id)
        VersionManager.save()
        // oh well
    } else if (ConfigManager.getSelectedServer() == uuid) {
        VersionManager.removeProfile(prevSelAcc.id)
        VersionManager.save()
        VersionManager.load()
        ConfigManager.setSelectedServer(VersionManager.getProfileByIndex(0).id)
        refreshAuthAccountSelected(VersionManager.getProfileByIndex(0).id)
    }
...
function refreshProfileSelected(uuid) {
    Array.from(document.getElementsByClassName('settingsAuthAccount')).map((val) => {
        const selBtn = val.getElementsByClassName('settingsAuthAccountSelect')[0]
        if (uuid === val.getAttribute('uuid')) {
            selBtn.setAttribute('selected', '')
...
const settingsCurrentProfiles = document.getElementById('settingsCurrentProfiles')
    ...
    authKeys.map((val) => {
        ...
        authAccountStr += `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${acc.name}" src="https://mc-heads.net/body/${acc.uuid}/60">
            </div>
...
function populateProfiles() {
    const selectedProfile = ConfigManager.getSelectedServer()

    let authAccountStr = ''
    let versionStr = ''
    let selectedIndex = ''

    let versionIndex = VersionManager.getIndex()
    let versions = versionIndex.getVersions()

    if (VersionManager.amountOfProfiles() == 1) {
        for (const vers of versions) {
            if (VersionManager.getVersion(0) == vers.getID()) {
                versionStr += `<option data-type="${vers.getType()}" value="${vers.getID()}" selected>${vers.getID()}</option>` 
            } else {
                versionStr += `<option data-type="${vers.getType()}" value="${vers.getID()}">${vers.getID()}</option>`
            }
        }
        authAccountStr += `<div class="settingsProfile" uuid="${VersionManager.getID(0)}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${VersionManager.getIcon(0)}" src="${VersionManager.getIcon(0)}">
            </div>
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">Profile Name</div>
                        <div class="settingsAuthAccountDetailValue">${VersionManager.getName(0)}</div>
                    </div>
                    <div class="settingsAuthAccountDetailPane">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsProfileSelect" ${selectedProfile === VersionManager.getID(0) ? 'selected>Selected Profile &#10004;' : '>Select Profile'}</button>
                </div>
            </div>
        </div>`
    } else {
        for (x = 0; x < VersionManager.amountOfProfiles(); ++x) {
            authAccountStr += `<div class="settingsProfile" uuid="${VersionManager.getID(x)}">
                <div class="settingsAuthAccountLeft">
                    <img class="settingsAuthAccountImage" alt="${VersionManager.getIcon(x)}" src="${VersionManager.getIcon(x)}">
                </div>
                <div class="settingsAuthAccountRight">
                    <div class="settingsAuthAccountDetails">
                        <div class="settingsAuthAccountDetailPane">
                            <div class="settingsAuthAccountDetailTitle">Profile Name</div>
                            <div class="settingsAuthAccountDetailValue">${VersionManager.getName(x)}</div>
                        </div>
                        <div class="settingsAuthAccountDetailPane">
                            <div class="settingsAuthAccountDetailTitle">Version</div>
                    ...
                    <div class="settingsAuthAccountActions">
                        <button class="settingsProfileSelect" ${selectedProfile === VersionManager.getID(x) ? 'selected>Selected Profile &#10004;' : '>Select Profile'}</button>
                        <div class="settingsAuthAccountWrapper">
                            <button class="settingsProfileDelete">Delete</button>
                        </div>
                    </div>
                </div>
            </div>`
    ...
    settingsCurrentProfiles.innerHTML = authAccountStr
}
...
/**
 * Prepare the profiles tab for display.
 */
        ...
        <div class="serverListingDetails">
            ...
            <div class="serverListingInfo">
                ...
                ${serv.isMainServer ? `<div class="serverListingStarWrapper">
                    ...
                    <span class="serverListingStarTooltip">Main Profile</span>
                </div>` : ''}
            </div>
        </div>
    `
