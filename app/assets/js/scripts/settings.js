    ...
    if (VersionManager.amountOfProfiles() == 1) {
        for ( const vers of versions) {
            if (VersionManager.getVersion(0) == vers.getID()) {
                versionStr += `<option value="${vers.getID()}" selected>${vers.getID()}</option>` 
            } else {
                versionStr += `<option value="${vers.getID()}">${vers.getID()}</option>`
            }
        }
...
function loadSelectedServerOnModsTab() {
    const serv = VersionManager.getProfile(ConfigManager.getSelectedServer().id)
